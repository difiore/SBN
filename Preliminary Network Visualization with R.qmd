...
# we can plot this directly with {visNetwork} and can event call igraph layout routines...
visIgraph(graph) %>%
...
# reach at different numbers of steps from "ego"
# note that `ego_size(graph, 1) - 1` is equivalent to degree!
# below, we scale reach by the number of nodes
# this is a measure of how many possible links ego could have
# note that in both the number and denominator, we subtract 1 to remove ego itself
...
Now, we put all these measures together in a data frame and add a "shape" variable for visualization in {visNetwork}

```{r}
centralities <- data.frame(id = names(degree), label = names(degree), degree, bw_weighted, bw_unweighted, eigen, hub, authority, closeness, reach_1, reach_2, reach_3, shape = "dot")
...
plot.igraph(graph, layout=l, 
            vertex.size=degree(graph) * 3,
            main="Degree")

plot.igraph(graph, layout=l, 
            vertex.size=eigen_centrality(graph)$vector * 15,
            main="Eigen")
...
plot.igraph(graph, layout=l, 
            vertex.size=hub_score(graph)$vector * 15,
            main="Hubs")

plot.igraph(graph, layout=l, 
            vertex.size=betweenness(graph) *3,
            main="Betweeness")
...
To calculate transitivity, another network level metric, we need the {statnet} package, which combines several other packages for network analysis, including {sna} and {network}

We first convert our igraph object to a network object using the package {intergraph}

```{r}
library(intergraph)
library(statnet)
network <- asNetwork(graph)
# we can plot network objects from within the {network} package
