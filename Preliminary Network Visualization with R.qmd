...
To calculate transitivity, another network level metric, we need to use the {statnet} package, which combines several other packages for network analysis, including {sna} and {network}

We first convert our {igraph} object to a network object using yet another network package {intergraph}....
...
# we can plot network objects from within the {statnet} package

# note that the plotting code below first introduces a new function, `rescale()`, that allows the user to specify a range of values over which to rescale a variable of interest
...
# the `gtrans()` function then calculates transitivity across the whole graph
